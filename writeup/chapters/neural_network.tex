...
\section{Variational Equation} \label{nn_variational}
...
or by defining the linear parabolic operator $\mathcal G$ \cite{zhao2025fractional}:

\[\mathcal G[v] = rv(t, S) - \frac{\partial v}{\partial t}(t, S) - \frac{\partial v}{\partial S}(t, S) r S - \frac{1}{2}\frac{\partial ^2v}{\partial S^2}(t, S) \sigma^2 S^2 \ge 0\]

which we could also write in terms of the Greeks:

\[\mathcal G[v] = rv(t, S) - \Theta - rS\Delta - \frac 12 \sigma^2 S^2\Gamma \ge 0\]

We can concisely express this inequality together with $(\dag)$:

\[
\min\left(\mathcal G[v], v(t, S) - g(S)\right) \ge 0
\]
...
which is the \textit{variational equation} \cite{jo2019variational} for the American option. Clearly, we have the terminal condition $v(T, S) = g(S)$.
...
We are now interested in finding the free boundary of the American put option, which is the boundary separating the exercise region and the continuation region. Due to the irregularity of our neural network solution, we will perform a soft classification instead of the classic hard classification for more intuitive visualisation. For hard classification, we would define the continuation region $A$ to be such that

\[A = \{(t, S) : u(t, S) > g(S)\} \cup \{(t, S) : g(S) = 0\}\]

where the first term indicates that we still have time value, and the second term immediately assigns at-the-money and out-of-the-money options in the continuation region as they are worthless under immediate exercise.

As there is a level of error in the neural network solution, we will extend the definition of the continuation region to a soft classification by considering the sigmoid function

\[ s(x) = \frac{1}{1 + e^{-x}}\]

and introducing two softness parameters $\tau_1, \tau_2 > 0$ to yield our continuation probability function on the relative difference from the payoff $d := \frac{u(t, S) - g(S)}{g(S)}$ as

...
We will also define a Martingale, which formalises the idea that future expectations of a process are equal to the current value (there is no drift in the process), which is a key concept in the risk-neutral pricing framework.
...
\begin{theorem}[Itô's lemma]
    Let $X = (X^1, \dots, X^d)$ be an Itô process and $f: \mathbb T \times \mathbb R^d \to \mathbb R \in \mathcal C^{1, 2}$ be a function that is once continuously differentiable in time and twice continuously differentiable in space. Then the process $f(t, X_t)$ is also an Itô process and satisfies
    \begin{equation}
        \begin{aligned}
